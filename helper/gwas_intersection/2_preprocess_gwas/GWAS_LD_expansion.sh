...
cut -f 2,7,8,22,28 gwas_catalog_v1.0-associations_e113_r2024-12-19.tsv> GWAS_associations.tsv
...
NR==1 {
    print $0, "BROAD_ANCESTRAL_CATEGORY";
    next;
}
NR>1 {
    print $0, anc[$1];
...
awk 'BEGIN { FS="\t"; OFS="\t" } NR > 1 { print $4, $0 }' reformatted_GWAS_catalog_wAncestries_ASN.tsv | sort -k1,1 > GWAS_prepared_sorted_ASN.tsv
...
##add back in header
header="rsID\tPUBMEDID\tSTUDY\tDISEASE_TRAIT\tP-VALUE\tLD_BUDDY\tR2"
...
# Prepare and Sort the GWAS File for Joining:
awk 'BEGIN { FS="\t"; OFS="\t" } NR > 1 { print $4, $0 }' reformatted_GWAS_catalog_wAncestries_AFR.tsv | sort -k1,1 > GWAS_prepared_sorted_AFR.tsv
...
# Prepare and Sort the GWAS File for Joining:
awk 'BEGIN { FS="\t"; OFS="\t" } NR > 1 { print $4, $0 }' reformatted_GWAS_catalog_wAncestries_EUR.tsv | sort -k1,1 > GWAS_prepared_sorted_EUR.tsv
